...
void solve()
{
    ...
    cin >> n;
    vi a(n), b(n);
    vi subSizeA((2 * n)+1,0), subSizeB((2 * n)+1,0);
    ...
    in_range(i, 0, n - 1)
    {
        ...
        {
            subSizeA[a[i]] = max(subSizeA[a[i]], cnt);
            cnt = 1;
        }
    }
    subSizeA[a[n - 1]] = max(subSizeA[a[n - 1]], cnt);
    cnt = 1;
    in_range(i, 0, n)
    {
        if (b[i] == b[i + 1])
        ...
        {
            subSizeB[b[i]] = max(subSizeB[b[i]], cnt);
            cnt = 1;
        }
    }
    subSizeB[b[n - 1]] = max(subSizeB[b[n - 1]], cnt);
    int ans = 0;
    in_range(i, 0, 2*n+1)
    {
        ans = max(ans, subSizeA[i] + subSizeB[i]);
    }
    cout<<ans << endl;
}
