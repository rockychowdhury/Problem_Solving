...
ll find_pos(vector<ll>&col,int idx, int x){
    int n = sz(col);
    auto it = lower_bound(col.rbegin(),col.rend()-idx,x);
    if(it==col.rend())return -1;
    ll pos = distance(col.begin(),it.base())-1;
    return pos;
}
...
In_range(i,0,m-1){
    ll present = col[i];
    ll target = n-col[i];
    if(target>present || !target)continue;
    ll search = find_pos(col,i+1,target);
    if(search!=-1){
        target--;
        ll possible =  sum[search]-sum[i];
        ll dif = search - i;
        possible -= (target*dif);
        way+=possible;
    }
    cout<<present<<" "<<++target<<" "<<search<<" "<<way<<endl;
}
// cout<<way*2<<endl;
}
